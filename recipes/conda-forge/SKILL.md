...
compatibility: >-
  ...
  rattler-build, conda-smithy, and cf-job-logs; conda-build for v0 recipes.
...
Work on the existing `recipe/recipe.yaml` (v1) or `recipe/meta.yaml` (v0), using the feedstock's configured build tool. [conda-forge documents both formats](https://conda-forge.org/docs/maintainer/adding_pkgs/#the-recipe-recipeyaml-or-metayaml). Preserve the format when fixing a build or updating a package; conversion is a separate change. When migration is requested or required by the feedstock, see [rattler-build-migration.md](references/rattler-build-migration.md).
...
**Important**: For a new contribution, start from the current upstream base without resetting or force-pushing your fork's `main`. Check `git status --short` and preserve unrelated uncommitted work before switching branches. Confirm that `upstream` points to the conda-forge feedstock, then create a new branch directly from the fetched base (`main` here; use the appropriate maintenance branch when needed):

```bash
git fetch upstream
git switch --no-track -c <branch> upstream/main
```

When updating an existing PR, keep its branch and preserve contributor and maintainer commits rather than starting over.

### Build Numbers

Use the [recipe update rules](https://conda-forge.org/docs/maintainer/updating_pkgs/#updating-recipes) against the PR's base recipe and the complete unmerged diff:

- New upstream package version: reset the build number to `0`.
- Same-version rebuild: increment the relevant base build number once.
- Follow-up fixes in an unmerged PR that already includes that increment: keep the candidate number unless that build has since been published. Do not increment once per commit.
- For multiple outputs or variant-specific offsets, preserve the intended arithmetic and check the affected published builds, including existing platforms that will rebuild. Packages are immutable, so the proposed artifact filenames must not collide with published ones.
...
Apply the minimal fix to the maintained inputs: the recipe, its scripts and patches, and `conda-forge.yml` when configuration needs to change. Regenerate `.ci_support` and generated CI files by rerendering; do not hand-edit them to work around an incorrect input.

### Test Locally

Use the feedstock's local build task or `build-locally.py` when available. For direct builds, choose a variant config that matches your local platform and use the tool matching the recipe format:

```bash
# v1 recipe.yaml
rattler-build build --recipe recipe -m .ci_support/<VARIANT>.yaml

# v0 meta.yaml, with conda-build installed
conda build recipe -m .ci_support/<VARIANT>.yaml -c conda-forge
```
...
Commit changes to the recipe and any maintained configuration first, then rerender (which auto-commits if there are changes). `conda-forge.yml` is a maintained input, not generated output. Review the generated diff and confirm that the matrix matches the intended change; see [rerendering a feedstock](https://conda-forge.org/docs/how-to/basics/rerender/).
